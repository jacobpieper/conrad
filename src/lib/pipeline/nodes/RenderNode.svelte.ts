...
export default class RenderNode extends Node {
	...
	async onRun(): Promise<void> {
		try {
			...
			const canvasObject = canvasManager.getCanvasByName(canvasId)
	...
	async onFrame(): Promise<void> {
		try {
			const imageData = this.getParameterByName('image').value as ImageData
			...
			console.error('Error in RenderNode onFrame:', error)
			throw error
		}
	}
}
